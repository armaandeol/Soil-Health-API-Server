...
def predict_soil_health(N, P, K, Moisture, Latitude, Longitude, Crop):
    ...
    try:
        ...
        # Create input array (using numpy instead of pandas)
        input_data = np.array([[N, P, K, Moisture, Latitude, Longitude, Crop_encoded]])
...
def batch_predict(data_list: List[SoilDataInput]):
    ...
    for item in data_list:
        try:
            ...
            logger.info(
                f"Prediction for {Crop} at ({Latitude}, {Longitude}): {prediction}, Deficiency: {deficiency_str}")
